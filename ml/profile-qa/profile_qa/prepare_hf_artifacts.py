...
def validate_promotion_metrics(
    *,
    ...
    """Require measurable promoted-model quality before release packaging."""
...
def load_model_provenance(
    ...
    return ModelProvenance(
        ...
        adapter_model_path=checkpoint_path,
...
def _require_disjoint_output(
    ...
    *input_paths: Path,
    ...
    """Prevent a release writer from deleting or copying over its inputs."""

    if output_path.is_symlink():
        raise ValueError(f"{operation} output path must not be a symlink: {output_path}")
    ...
    for input_path in input_paths:
        ...
        if (
            ...
            raise ValueError(
                f"{operation} output path must not overlap release input "
                f"{input_path}: {output_path} and {input_path}"
            )


def _preflight_payload_outputs(
    args: argparse.Namespace,
    browser_dir: Path,
    provenance: ModelProvenance,
) -> None:
    """Reject unsafe output layouts before either payload writer mutates disk."""
    ...
    release_inputs = (
        ...
        Path(args.lineage_file),
        browser_dir,
        provenance.adapter_model_path,
    )
    output_dir = Path(args.output_dir)
    for payload_name in ("model", "dataset"):
        _require_disjoint_output(
            output_dir / payload_name,
            *release_inputs,
            operation=f"{payload_name} payload preparation",
        )


def _load_release_inputs(args: argparse.Namespace) -> ReleaseInputs:
    ...
    _preflight_payload_outputs(args, browser_dir, provenance)
...
def prepare_model_payload(args: argparse.Namespace) -> Path:
    release_inputs = _load_release_inputs(args)

    model_output_dir = Path(args.output_dir) / "model"
    _copy_tree_contents(release_inputs.browser_dir, model_output_dir)

    _write_model_card(
        model_output_dir / "README.md",
        model_repo_id=args.model_repo_id,
        dataset_repo_id=args.dataset_repo_id,
        baseline_test=release_inputs.reports.baseline_test,
        promoted_validation=release_inputs.reports.promoted_validation,
        promoted_test=release_inputs.reports.promoted_test,
        provenance=release_inputs.provenance,
    )
...
def prepare_dataset_payload(args: argparse.Namespace) -> Path:
    ...
    loaded_reports = [
        release_inputs.reports.baseline_test,
        release_inputs.reports.promoted_validation,
        release_inputs.reports.promoted_test,
    ]
